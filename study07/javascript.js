...
tListOl = document.querySelectorAll('.w_list'); // 할 일 리스트 불러오기
cListOl = document.querySelectorAll('.c_list'); // 할 일 리스트 불러오기

// 프로그램을 처음부터 다시 시작
function init() {
    rendertList(t_lists, tListOl); // 할 일 리스트 출력
    rendercList(); // 완료한 일 리스트 출력 
}

// tList 동적으로 출력
function rendertList(lists, ListOl) {
    var wwork_li_html = '';
    for(var i = 0; i < lists.length; i++){
        wwork_li_html = wwork_li_html + `
        <li>
        <div class="works">${(left)[i].todo}</div>
